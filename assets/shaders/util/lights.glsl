...
vec3 calcLight(material m, lightSource src, vec3 viewPos, vec3 pos, vec3 normalVec, vec2 uv) {
    ...
    return (amb * getAmbientColor(m, uv) + diff * getDiffuseColor(m, uv) + spec * getSpecularColor(m, uv)) * getColor(m, uv).rgb;
}
